...
# -*- coding: utf-8 -*-
"""
v13.3 Bayesian-coupled full-diagnostics block discovery.
...
except Exception:  # pragma: no cover - v13.3 has numpy fallbacks for notebook portability.
...
DEFAULT_CONFIG: Dict = {
    "version": "v13.3_full_diagnostics_block_discovery",
    "toy": {
        "seed": 20270714,
        "n_trials": 120,
        "n_time": 90,
    ...
    "advanced_methods": {
        ...
        "preferred_uses_block_posterior_F": True,
        "spectral_n_init": 32,
        "spatial_tau": 0.18,
        "spatial_floor": 0.20,
        "eb_weight": 0.35,
...
# =============================================================================
# v13.3 PSM / spectral / BGM / EB-shrinkage block summaries
# =============================================================================
...
def advanced_block_method_comparison(
    ...
    if K == 0 or len(coassoc_df) == 0:
        return pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), {}
    C = normalize_affinity(coassoc_df.drop(columns=["component", "gaussian_center"]).to_numpy(float))
    ...
    def add_candidate(method: str, F: int, labels: np.ndarray, A: np.ndarray, extra: Optional[Dict] = None):
    ...
    if len(cand_df):
        ...
        preferred = str(cfg["advanced_methods"]["preferred_method"])
        if preferred not in set(cand_df["method"]):
            preferred = "psm_spatial_spectral" if "psm_spatial_spectral" in set(cand_df["method"]) else str(cand_df.iloc[cand_df["method_score"].idxmax()]["method"])
        cand_df["selected_v13_3"] = False
        pref_rows = cand_df[cand_df["method"] == preferred]
        posterior_F = None
        if bool(cfg["advanced_methods"].get("preferred_uses_block_posterior_F", True)) and len(block_df):
            try:
                posterior_F = int(block_df.loc[block_df["selected"], "F"].iloc[0])
            except Exception:
                posterior_F = None
        if len(pref_rows):
            pref_posterior_rows = pref_rows[pref_rows["F"] == posterior_F] if posterior_F is not None else pd.DataFrame()
            if len(pref_posterior_rows):
                cand_df.loc[pref_posterior_rows["method_score"].idxmax(), "selected_v13_3"] = True
            else:
                cand_df.loc[pref_rows["method_score"].idxmax(), "selected_v13_3"] = True
        else:
            cand_df.loc[cand_df["method_score"].idxmax(), "selected_v13_3"] = True
    assign_df2 = pd.DataFrame(assign_rows)
    if len(assign_df2) and len(cand_df):
        selected_keys = cand_df[cand_df["selected_by_method"]][["method", "F", "requested_F"]].drop_duplicates()
        assign_df2 = assign_df2.merge(selected_keys.assign(selected_by_method=True), on=["method", "F", "requested_F"], how="left")
        assign_df2["selected_by_method"] = assign_df2["selected_by_method"].fillna(False).astype(bool)
        selected_v13 = cand_df[cand_df["selected_v13_3"]][["method", "F", "requested_F"]].drop_duplicates()
        assign_df2 = assign_df2.merge(selected_v13.assign(selected_v13_3=True), on=["method", "F", "requested_F"], how="left")
        assign_df2["selected_v13_3"] = assign_df2["selected_v13_3"].fillna(False).astype(bool)

    method_summary = (
        cand_df[cand_df["selected_by_method"] | cand_df["selected_v13_3"]]
        .sort_values(["method", "selected_v13_3", "method_score"], ascending=[True, False, False])
        .reset_index(drop=True)
...
def full_space_diagnostics(
    ...
    np.savez_compressed(
        outdir / "v13_3_latent_matrices.npz",
        W_raw=W_est,
        H_raw=H_est,
        W_localized=W_loc,
        H_localized=H_loc,
...
def save_plots(
    ...
    plt.savefig(outdir / "v13_3_identifiability_rank.png", dpi=dpi); plt.close()
    ...
    plt.legend(); plt.tight_layout()
    plt.savefig(outdir / "v13_3_iterative_K_trace.png", dpi=dpi); plt.close()
    ...
    plt.legend(); plt.tight_layout()
    plt.savefig(outdir / "v13_3_iterative_gain_trace.png", dpi=dpi); plt.close()
    ...
    if len(cand_df):
        ...
        plt.legend(); plt.tight_layout()
        plt.savefig(outdir / "v13_3_residual_reliability_trace.png", dpi=dpi); plt.close()
    ...
    plt.legend(); plt.tight_layout()
    plt.savefig(outdir / "v13_3_localized_component_centers.png", dpi=dpi); plt.close()
    ...
    if len(block_df):
        ...
        plt.legend(); plt.tight_layout()
        plt.savefig(outdir / "v13_3_F_block_posterior.png", dpi=dpi); plt.close()
    ...
    if comp_df is not None and len(comp_df):
        ...
        plt.legend(); plt.tight_layout()
        plt.savefig(outdir / "v13_3_component_inclusion_posterior.png", dpi=dpi); plt.close()
    ...
    if block_post_df is not None and len(block_post_df):
        ...
        plt.legend(); plt.tight_layout()
        plt.savefig(outdir / "v13_3_F_posterior_mass.png", dpi=dpi); plt.close()
    ...
    if coassoc_df is not None and len(coassoc_df):
        ...
        plt.tight_layout()
        plt.savefig(outdir / "v13_3_block_coassociation_heatmap.png", dpi=dpi); plt.close()
    ...
    if method_summary_df is not None and len(method_summary_df):
        ...
        plt.bar(plot_df["method"], plot_df["method_score"])
        plt.xticks(rotation=25, ha="right")
        plt.ylabel("unsupervised method score")
        plt.title("v13.3 selected-method comparison")
        plt.tight_layout()
        plt.savefig(outdir / "v13_3_method_score_comparison.png", dpi=dpi); plt.close()

        if "ARI_nearest_true_group_center" in plot_df.columns:
            plt.figure(figsize=(9.0, 4.8))
            plt.bar(plot_df["method"], plot_df["ARI_nearest_true_group_center"])
            plt.xticks(rotation=25, ha="right")
            plt.ylim(0, 1.05)
            plt.ylabel("ARI vs nearest true group center")
            plt.title("v13.3 method recovery diagnostic")
            plt.tight_layout()
            plt.savefig(outdir / "v13_3_method_ARI_comparison.png", dpi=dpi); plt.close()
    ...
    if method_candidates_df is not None and len(method_candidates_df):
        ...
        plt.title("v13.3 spectral candidate surface")
        plt.legend(fontsize=8)
        plt.tight_layout()
        plt.savefig(outdir / "v13_3_spectral_candidate_surface.png", dpi=dpi); plt.close()
    ...
    if method_affinities:
        for name, A in method_affinities.items():
            ...
            plt.xlabel("component"); plt.ylabel("component")
            plt.title(f"v13.3 affinity: {name}")
            plt.tight_layout()
            plt.savefig(outdir / f"v13_3_affinity_{name}.png", dpi=dpi); plt.close()
...
def write_report(
    ...
    selected_adv = method_candidates_df[method_candidates_df["selected_v13_3"]].copy() if len(method_candidates_df) and "selected_v13_3" in method_candidates_df.columns else pd.DataFrame()
    selected_adv_row = selected_adv.iloc[0].to_dict() if len(selected_adv) else {}
    report = {
        "version": cfg["version"],
        "elapsed_sec": float(elapsed),
        "design_notes": [
            "v13.3 keeps the v13.1/v13.2 residual-driven iterative K core.",
            "v13.3 compares several block summaries on the same posterior evidence.",
            ...
            "BayesianGaussianMixture provides a truncated-DP-style comparison route."
        ],
        "truth": {
        ...
        "selected": {
            ...
            "v13_3_selected_method": selected_adv_row.get("method"),
            "v13_3_selected_F": int(selected_adv_row["F"]) if "F" in selected_adv_row and pd.notna(selected_adv_row["F"]) else None,
            "v13_3_selected_method_score": float(selected_adv_row["method_score"]) if "method_score" in selected_adv_row and pd.notna(selected_adv_row["method_score"]) else None,
        ...
        "v13_3_method_summary": method_summary_df.to_dict(orient="records") if len(method_summary_df) else [],
        "v13_3_method_candidates_selected": method_candidates_df[method_candidates_df["selected_by_method"]].to_dict(orient="records") if len(method_candidates_df) and "selected_by_method" in method_candidates_df.columns else [],
        "space_diagnostics": {
    ...
    with open(outdir / "v13_3_report.json", "w", encoding="utf-8") as f:
        json.dump(report, f, indent=2, ensure_ascii=False)

    lines = []
    lines.append("# v13.3 full diagnostics block discovery report\n")
    ...
    lines.append(f"- v13.3 selected method: {selected_adv_row.get('method', 'NA')}\n")
    lines.append(f"- v13.3 selected F: {selected_adv_row.get('F', 'NA')}\n")
    ...
    lines.append("\n\n## v13.3 method summary\n")
    lines.append(method_summary_df.to_string(index=False) if len(method_summary_df) else "No v13.3 method summary")
    lines.append("\n\n## v13.3 selected method candidates\n")
    lines.append(method_candidates_df[method_candidates_df["selected_by_method"]].to_string(index=False) if len(method_candidates_df) and "selected_by_method" in method_candidates_df.columns else "No v13.3 method candidates")
    lines.append("\n\n## Iterative K trace\n")
    ...
    with open(outdir / "v13_3_report.md", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    return report
...
def run_pipeline(cfg: Dict, outdir: Path):
    ...
    audit_df.to_csv(outdir / "v13_3_identifiability_audit.csv", index=False)

    iter_res = true_iterative_K_expansion(X, signal, cfg, outdir)
    iter_res["trace"].to_csv(outdir / "v13_3_iterative_trace.csv", index=False)
    iter_res["candidates"].to_csv(outdir / "v13_3_candidate_trace.csv", index=False)
    ...
    local["components_df"].to_csv(outdir / "v13_3_gaussian_components.csv", index=False)
    ...
    comp_df.to_csv(outdir / "v13_3_component_posterior.csv", index=False)
    ...
    block_df.to_csv(outdir / "v13_3_block_candidates.csv", index=False)
    assign_df.to_csv(outdir / "v13_3_block_assignment.csv", index=False)
    coassoc_df.to_csv(outdir / "v13_3_block_coassociation.csv", index=False)
    block_post_df.to_csv(outdir / "v13_3_block_posterior_summary.csv", index=False)
    ...
    method_summary_df.to_csv(outdir / "v13_3_method_summary.csv", index=False)
    method_candidates_df.to_csv(outdir / "v13_3_method_candidates.csv", index=False)
    method_assign_df.to_csv(outdir / "v13_3_method_assignment.csv", index=False)
    for name, A in method_affinities.items():
        pd.DataFrame(A, columns=[f"component_{j}" for j in range(A.shape[1])]).to_csv(
            outdir / f"v13_3_affinity_{name}.csv",
            index=False,
        )
    ...
    space_diag_df.to_csv(outdir / "v13_3_space_diagnostics.csv", index=False)
    subspace_angles_df.to_csv(outdir / "v13_3_subspace_angles.csv", index=False)
    center_match_df.to_csv(outdir / "v13_3_center_matching.csv", index=False)
    alignment_df.to_csv(outdir / "v13_3_component_alignment.csv", index=False)
...
def main():
    ap = argparse.ArgumentParser(description="v13.3 full-diagnostics Bayesian-coupled iterative block discovery")
    ap.add_argument("--config", type=str, default=None)
    ap.add_argument("--outdir", type=str, default="v13_3_outputs")
