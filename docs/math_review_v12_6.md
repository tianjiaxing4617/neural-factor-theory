...
$$
\text{data} \longrightarrow \text{reliable subspace} \longrightarrow \text{localized components} \longrightarrow \text{block graph}.
$$
...
$$
X_{\mathrm{flat}} = H W^\top + \varepsilon,
$$
...
$$
W_g = [w_{g,1},\ldots,w_{g,C}] \in \mathbb{R}^{N \times C},
$$
...
$$
r_{\mathrm{eff}}(W_g) = \exp\!\left( -\sum_i p_i \log p_i \right).
$$
...
$$
\sigma_i \left( (\widehat{U}^{(s_1)}_K)^\top \widehat{U}^{(s_2)}_K \right),
$$
...
$$
(\widehat{h}_{q+1}, \widehat{w}_{q+1}) = \mathrm{Factorize}(R_q).
$$
...
$$
X = H_K W_K^\top + \varepsilon, \qquad \varepsilon \sim \mathcal{N}(0,\sigma^2 I),
$$

则负对数似然的主要部分为

$$
-\log p(X \mid H_K,W_K,\sigma^2) \propto \frac{1}{2\sigma^2} \left\|X-H_K W_K^\top\right\|_F^2 + \frac{MN}{2}\log\sigma^2.
$$
...
$$
S_K = \alpha \cdot \mathrm{Reliability}(K) + \beta \cdot \mathrm{ExplainedVar}(K) - \gamma \cdot \mathrm{Penalty}(K).
$$
...
$$
\phi_j(n) = \exp\!\left( -\frac{(z_n-c_j)^2}{2\sigma_j^2} \right).
$$
...
$$
Q^\star = \arg\max_{Q^\top Q=I} \sum_{k=1}^{K} \mathrm{Locality} \left( \widehat{w}_k(Q) \right).
$$
...
$$
\mathrm{corr}(1-z,y) = -\mathrm{corr}(z,y),
$$

所以两边取绝对值后永远相等：

$$
\left| \mathrm{corr}(1-z,y) \right| = \left| \mathrm{corr}(z,y) \right|.
$$
...
$$
\mathrm{dCor}^2(h_k,Y) = \frac{ \mathrm{dCov}^2(h_k,Y) }{ \mathrm{dVar}(h_k)\mathrm{dVar}(Y) }.
$$
...
$$
O^W_{ij} = \frac{ \sum_n |w_i(n)| |w_j(n)| }{ \sqrt{ \sum_n w_i(n)^2 } \sqrt{ \sum_n w_j(n)^2 } }.
$$
...
$$
P^{H \mid Y}_{ij} = \mathrm{Dep}(h_i,h_j \mid Y).
$$
...
$$
S_{ij}^{\mathrm{split}} = \mathbb{P}_{s} \left[ i \sim j \text{ in split } s \right].
$$

理想的 co-association matrix 应该是：

$$
C_{ij} = \frac{1}{S} \sum_{s=1}^{S} \mathbf{1} \left[ c_i^{(s)} = c_j^{(s)} \right].
$$
...
$$
\mathrm{Within}(B) = \sum_{b \in B} \frac{1}{|b|(|b|-1)} \sum_{i,j \in b,\,i\ne j} G_{ij},
$$
...
$$
X^{\mathrm{null}} = \mathrm{Shuffle}(X),
$$

并重复完整 pipeline 得到 null score：

$$
S^{\mathrm{null}}_K = \mathrm{PipelineScore}(X^{\mathrm{null}},K).
$$

真实 score 的显著性为

$$
p_K = \frac{ 1 + \sum_{b=1}^{B} \mathbf{1} \left[ S_{K,b}^{\mathrm{null}} \ge S_K \right] }{ B+1 }.
$$
...
$$
\mathbb{E} \left[ S_{\mathrm{val}}(\widehat{K}) \right] > \mathbb{E} \left[ S_{\mathrm{test}}(\widehat{K}) \right].
$$
...
$$
R^2(K_{\mathrm{selected}}) \quad \text{而不是} \quad R^2(K_{\mathrm{accepted,max}}).
$$
...
$$
\boxed{ \text{v12.6 recovers reliable neural subspaces and localized block candidates,} }
$$

而不是：

$$
\boxed{ \text{v12.6 uniquely identifies all true latent factors.} }
$$
